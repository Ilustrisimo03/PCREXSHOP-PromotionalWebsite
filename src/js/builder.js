            ...
            if (product) {
                ...
                slotDiv.innerHTML = `
                    <div class="flex items-center gap-4">
                        <i class="fas ${slot.icon} text-xl text-gray-400 w-6 text-center"></i>
                        <h3 class="text-base font-bold text-gray-600">${slot.name} ${requiredText}</h3>
                    </div>
                    <button class="choose-btn BG-gradient text-white font-bold py-2 px-4 rounded-lg transition-colors hover:opacity-90" data-slot-id="${slot.id}" data-type="${slot.type}" data-subtype="${slot.subType || ''}">
                        Choose <i class="fas fa-chevron-right ml-2 text-sm"></i>
                    </button>
                `;
            }
